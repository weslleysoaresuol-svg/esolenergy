...
function AuthPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.signUp({
      email, password,
      options: { emailRedirectTo: window.location.origin + "/app", data: { full_name: nome } },
    });
    setLoading(false);
    if (error) toast.error(error.message);
    else { toast.success("Cadastro criado! Faça login."); }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-navy via-navy-deep to-navy flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-8">
        <Link to="/" className="flex justify-center mb-6">
          <img src={logo} alt="ESOL" className="h-14 w-auto" />
        </Link>
        <h1 className="text-2xl font-bold text-center text-navy mb-1">Acesso da equipe</h1>
        <p className="text-sm text-center text-muted-foreground mb-6">Administradores e corretores parceiros</p>
        ...
        <Tabs defaultValue="login">
          ...
          <TabsContent value="login">
            <form onSubmit={handleLogin} className="space-y-3 mt-3">
              ...
              <Button type="submit" disabled={loading} className="w-full bg-navy hover:bg-navy-deep">Entrar</Button>
            </form>
          </TabsContent>
          <TabsContent value="signup">
            <form onSubmit={handleSignup} className="space-y-3 mt-3">
              ...
              <Button type="submit" disabled={loading} className="w-full bg-sun-deep hover:bg-sun text-navy">Criar conta</Button>
              <p className="text-xs text-muted-foreground text-center">Após criar a conta, complete seu perfil para começar a vender.</p>
            </form>
          </TabsContent>
        </Tabs>

        <Link to="/" className="block text-center text-sm text-muted-foreground mt-6 hover:text-navy">← Voltar ao site</Link>
      </div>
    </div>
  );
}
