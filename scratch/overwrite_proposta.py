...
const FINANCEIRAS = {
  solfacil: {
    ...
    cetAnual: 17.3,
    label: "Solfácil",
    info: "Taxa CET de 1.34% a.m. Prazo de até 84 meses."
  },
  bv: {
    id: "bv",
    nome: "Banco BV",
    ...
    cetAnual: 18.8,
    label: "Banco BV",
    info: "Taxa CET de 1.45% a.m. Prazo de até 72 meses."
  },
  sicredi: {
    id: "sicredi",
    nome: "Sicredi",
    ...
    cetAnual: 16.1,
    label: "Sicredi",
    info: "Taxa CET de 1.25% a.m. Prazo de até 60 meses para cooperados."
  },
  santander: {
    id: "santander",
    nome: "Santander",
    ...
    cetAnual: 18.3,
    label: "Santander",
    info: "Taxa CET de 1.41% a.m. Prazo de até 72 meses."
  }
};
...
export function PropostaView({ proposta: p, parceiro, cliente, publico, onAceitar, onRecusar }: PropostaViewProps) {
  ...
  const FINANCEIRAS_ESTIMADO = useMemo(() => {
    if (bancos.length === 0) return FINANCEIRAS;
    const dict: any = { ...FINANCEIRAS };
    
    bancos.forEach((b) => {
      const cleanKey = b.nome
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\\u0300-\\u036f]/g, "")
        .replace(/banco/g, "")
  ...
  }, [p.preco_total, p.economia_mensal, p.economia_anual, selectedFin, selectedPrazo, dadosAprovados, bancos]);
  ...
  return (
    <div className="bg-[#000512] text-white font-sans min-h-screen relative overflow-hidden" style={{ WebkitPrintColorAdjust: "exact", printColorAdjust: "exact" } as React.CSSProperties}>
      ...
      <div className="absolute inset-0 pointer-events-none opacity-[0.015] bg-repeat" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)'/%3E%3C/svg%3E")` }} />
        ...
        <div className="max-w-5xl mx-auto w-full flex-1 flex flex-col justify-between gap-12">
          ...
          <div className="grid md:grid-cols-12 gap-8 items-center my-auto">
            ...
            <div className="md:col-span-7 space-y-6">
              <div className="inline-flex items-center gap-2 bg-sun/10 text-sun px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest border border-sun/20 animate-fade-in">
                <Sun className="w-3.5 h-3.5 animate-spin" style={{ animationDuration: "20s" }} /> ENERGIA SOLAR FOTOVOLTAICA
              </div>
              
              <h1 className="font-display text-4xl md:text-6xl font-black leading-[1.08] tracking-tight animate-fade-up">
              ...
              <p className="text-slate-300 text-base md:text-lg leading-relaxed max-w-xl animate-fade-up">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-white/[0.02] border border-white/[0.06] backdrop-blur-xl rounded-3xl p-6 md:p-8 space-y-6">
              ...
              <div className="space-y-4 text-sm font-medium">
                ...
                {componentesKit.modulos && (
                  <div className="flex justify-between items-center border-b border-white/5 pb-2">
                    <div className="flex flex-col">
                      <span className="font-bold text-white">{componentesKit.modulos.marca} ({componentesKit.modulos.potencia}W)</span>
                      <span className="text-xs text-white/50">{componentesKit.modulos.quantidade} Módulos Fotovoltaicos</span>
                    </div>
                    <span className="text-xs font-mono font-bold text-white/40">Garantia: {p.kit_garantia_modulos_anos || 25} Anos</span>
                  </div>
                )}

                {componentesKit.inversor && (
                  <div className="flex justify-between items-center border-b border-white/5 pb-2">
                    <div className="flex flex-col">
                      <span className="font-bold text-white">{componentesKit.inversor.marca} ({componentesKit.inversor.modelo})</span>
                      <span className="text-xs text-white/50">{componentesKit.inversor.tipo} de última geração</span>
                    </div>
                    <span className="text-xs font-mono font-bold text-white/40">Garantia: {p.kit_garantia_inversor_anos || 10} Anos</span>
                  </div>
                )}

                {componentesKit.estrutura && (
                  <div className="flex justify-between items-center pb-2">
                    <div className="flex flex-col">
                      <span className="font-bold text-white">Estruturas de Fixação</span>
                      <span className="text-xs text-white/50">Projetadas para telhado {p.tipo_instalacao || "residencial"}</span>
                    </div>
                    <span className="text-xs font-mono font-bold text-white/40">Alumínio / Inox</span>
                  </div>
                )}
              </div>
            </div>
...
with open("src/components/PropostaView.tsx", "w", encoding="utf-8") as f:
    f.write(code_content)

print("PropostaView.tsx overwritten successfully!")
